    ...
    while($hids=$rows->fetch_assoc())
       ...
       if($_SERVER['REQUEST_METHOD']=="POST")
       {
            ...
            $contact=$_POST['contact'];
             if( !$result=$conn->query("select quantity from bloodbank where blood_type='$blood'"))
          die('select from bloodbank '.$conn->error);
           $rows=$result->fetch_assoc();
           $avalibleq=$rows['quantity'];
           if($avalibleq<$quantity)
           {
             $sql2="select name,contact from donor where blood_type='$blood' ";
             if(!$result2=$conn->query($sql2))
             die($conn->error);
             $ans=$result2->fetch_assoc();
            $error="We have only ".$avalibleq." ml of blood in blood bank contact <strong> ".$ans['name']."</strong> contact Number <strong> ".$ans['contact']."</strong>";
...
<div class="container">
  <h2><strong>ADD Reciever</strong></h2>
...
<?php
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" id="pwd" placeholder="Enter Age" name="age" pattern="[0]{1}[1-9]{1}|[1-9]{1}[0-9]{1}" required>
    </div>
    <div class="form-group">
